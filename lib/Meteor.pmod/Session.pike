...
void register_new_id() {
    ...
    //if (autoclose)
    //	werror("Creating new autoclosing Stream for %O\n", connection_id);
	...
	stream = Meteor.Stream(connection_id->connection(), stream_close,
			       stream_error, autoclose);
    } else {
	//werror("creating utf8 stream\n");
	headers["Content-Type"] = "text/plain; charset=utf-8";
	stream = Meteor.UTF8Stream(connection_id->connection(), stream_close,
				   stream_error, autoclose);
    }
	...
	stream->feed(connection_id->make_response_headers(headers)
		     + sprintf("%x\r\n%s\r\n", 20*sizeof(keepalive_packet),
			       keepalive_packet*20));
    }
    ...
    while (!queue->isEmpty()) {
	mixed o = queue->shift();
	if (object_program(o) == Serialization.Atom)
	    stream->write(o->render());
	else stream->write(o);
    }
